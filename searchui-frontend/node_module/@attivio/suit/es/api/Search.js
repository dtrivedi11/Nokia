...
var Search = function () {
  ...
  Search.prototype.search = function search(request, updateResults) {
    ...
    if (this.searchEngineType === 'elastic') {
      QueryRequestToElastic(jsonRequest, '' + this.baseUri, this.customOptions, function (err, searchResponse) {
        if (err) {
      ...
      QueryRequestToSolr(jsonRequest, '' + this.baseUri, this.customOptions, function (err, searchResponse) {
        if (err) {
          updateResults(null, err);
        }
        updateResults(searchResponse, null);
      ...
      fetch(fetchRequest).then(function (response) {
        if (response.ok) {
          response.json().then(function (jsonResponse) {
            var searchResponse = QueryResponse.fromJson(jsonResponse);
            updateResults(searchResponse, null);
