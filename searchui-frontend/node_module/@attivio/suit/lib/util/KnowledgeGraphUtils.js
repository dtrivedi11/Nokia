...
exports.default = exports.GraphDefinition = undefined;
...
var GraphDefinition = exports.GraphDefinition = function GraphDefinition(nodes, edges) {
...
var KnowledgeGraphUtils = (_temp = _class2 = function () {
  ...
  KnowledgeGraphUtils.buildQuery = function buildQuery(docId, table, tableField, linkingFields, maxLinkedDocs, entityName, entityValue) {
    ...
    if (entityName && entityValue) {
      ...
      return 'BOOST("' + entityName + '":"' + entityValue + '", ' + _FieldNames2.default.ID + ':"' + escapedDocId + '")';
    }
    var primaryQuery = 'QUERY("' + _FieldNames2.default.ID + ':\\"' + escapedDocId + '\\"", qlang=advanced)';
  ...
  KnowledgeGraphUtils.makeDocNode = function makeDocNode(doc, id) {
    ...
    var label = _StringUtils2.default.wrapLabel(doc.getFirstValue('title'));
    var docId = doc.getFirstValue(_FieldNames2.default.ID);
    ...
    var node = new _GraphNode2.default(id, label, title, 'document');
    node.physics = false;
    if (isPrimary) {
      node.borderWidth = 1;
      node.color = {
  ...
  KnowledgeGraphUtils.makeLinkingNode = function makeLinkingNode(fieldName, fieldValue, id) {
    ...
    var node = new _GraphNode2.default(id, _StringUtils2.default.wrapLabel(fieldValue), title, fieldName);
    return node;
  };
  ...
  KnowledgeGraphUtils.searchResultsToGraph = function searchResultsToGraph(mainDoc, linkingFields) {
    ...
    allDocs.forEach(function (doc) {
      var existingDoc = filteredDocs.find(function (filteredDoc) {
        return doc.getFirstValue(_FieldNames2.default.ID) === filteredDoc.getFirstValue(_FieldNames2.default.ID);
      });
      if (!existingDoc) {
    ...
    filteredDocs.forEach(function (doc) {
      ...
      nodeId += 1;
      nodes.push(docNode);
      docNodes.set(doc.getFirstValue(_FieldNames2.default.ID), docNode);
          ...
          fieldValues.forEach(function (fieldValue) {
            ...
            if (existingNode) {
              ...
              if (!existingEdge) {
                ...
                edges.push(new _GraphEdge2.default(docNode.id, existingNode.id));
              }
            } else {
              ...
              edges.push(new _GraphEdge2.default(docNode.id, newNode.id));
            }
          });
        }
      });
  ...
  KnowledgeGraphUtils.calculateGraphOptions = function calculateGraphOptions() {
    ...
    var colors = _AuthUtils2.default.getEntityColors();
...
exports.default = KnowledgeGraphUtils;